...
const YT_PLAYER_STYLES = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .ytp-btn-play { background: var(--ytp-accent); color: #fff; }
  ...
  .ytp-body { display: grid; grid-template-columns: 1fr; }

  .ytp-body.has-sidebar { grid-template-columns: 1fr 280px; }
  ...
  .ytp-tabs { display: flex; border-bottom: 1px solid var(--ytp-border); }

  .ytp-tab {
    flex: 1; padding: 12px 8px;
    background: none; border: none;
    color: var(--ytp-text-sec); font-size: 12px; font-weight: 600;
  ...
  .ytp-list {
    flex: 1; min-height: 150px; max-height: 320px;
  ...
  .ytp-empty { text-align: center; padding: 32px 12px; color: var(--ytp-text-dim); font-size: 13px; }
  ...
  .ytp-clear-btn {
    ...
    border: none; border-top: 1px solid var(--ytp-border);
    background: none; color: var(--ytp-text-dim);
...
class YTPlayer extends HTMLElement {
  static get observedAttributes() { return ['playlist', 'history']; }
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    if (name === 'playlist' || name === 'history') this.#updateSidebar();
  }
  ...
  #loadState() {
    try {
      this.#playlist = JSON.parse(localStorage.getItem(this.#storageKey('playlist'))) || [];
      this.#history = JSON.parse(localStorage.getItem(this.#storageKey('history'))) || [];
    } catch { this.#playlist = []; this.#history = []; }
  }
  ...
  #play(id) {
    ...
    if (!this.#apiReady) { this.#queue.push(id); return; }
  ...
  #addToPlaylist(id) {
    if (this.#playlist.some(v => v.id === id)) { this.#showError('Already in playlist'); return; }
  ...
  #clearPlaylist() { this.#playlist = []; this.#saveState(); this.#renderList('playlist'); }
  ...
  #clearHistory() { this.#history = []; this.#saveState(); this.#renderList('history'); }
  ...
  #renderList(type) {
    ...
    if (items.length === 0) {
      ...
      clearBtn.classList.remove('visible'); return;
    }
    ...
    list.querySelectorAll('.ytp-item').forEach(el => {
      el.addEventListener('click', (e) => { if (e.target.closest('.ytp-item-remove')) return; this.#play(el.dataset.id); });
    });

    if (type === 'playlist') {
      list.querySelectorAll('.ytp-item-remove').forEach(btn => {
        btn.addEventListener('click', (e) => { e.stopPropagation(); this.#removeFromPlaylist(btn.dataset.id); });
      });
    }
  }
  ...
  #bindEvents() {
    const els = this.#els;
    els.playBtn.addEventListener('click', () => { const id = this.#getInputId(); if (id) { this.#play(id); els.input.value = ''; } });
    els.addBtn.addEventListener('click', () => { const id = this.#getInputId(); if (id) { this.#addToPlaylist(id); els.input.value = ''; } });
    els.input.addEventListener('keydown', (e) => { if (e.key === 'Enter') els.playBtn.click(); });
    ...
    els.tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        this.#root.querySelectorAll('.ytp-tab-content').forEach(c => { c.classList.toggle('active', c.dataset.tab === target); });
      });
    });
  }

  #ensureIFrameAPI() {
    if (window.YT && window.YT.Player) { this.#onAPIReady(); return; }
    if (document.querySelector('script[src*="youtube.com/iframe_api"]')) return;
    const tag = document.createElement('script');
    tag.src = 'https://www.youtube.com/iframe_api';
    document.head.appendChild(tag);
    const check = setInterval(() => {
      if (window.YT && window.YT.Player) { clearInterval(check); this.#onAPIReady(); }
    }, 200);
  }
